...
int FolderModel::depthFromPath(const QString &path) {
  ...
  if (p.startsWith(QLatin1Char('['))) {
    ...
    if (closeBracket >= 0 && closeBracket + 1 < p.size() &&
        p[closeBracket + 1] == QLatin1Char('/'))
...
FolderModel::FolderModel(QObject *parent)
    : QAbstractItemModel(parent), m_root(new TreeNode) {
...
FolderModel::~FolderModel() { delete m_root; }
...
FolderRole FolderModel::classifyFolder(const QString &path) {
  ...
  if (lower == QStringLiteral("sent") ||
      lower == QStringLiteral("sent items") ||
      lower == QStringLiteral("sent mail"))
    return FolderRole::Sent;
  if (lower == QStringLiteral("drafts"))
    return FolderRole::Drafts;
  if (lower == QStringLiteral("trash") || lower == QStringLiteral("deleted") ||
      lower == QStringLiteral("deleted items") ||
      lower == QStringLiteral("bin"))
...
int FolderModel::folderOrder(FolderRole role) {
  switch (role) {
  case FolderRole::Inbox:
    return 0;
  case FolderRole::Starred:
    return 1;
  case FolderRole::Drafts:
    return 2;
  case FolderRole::Sent:
    return 3;
  case FolderRole::Archive:
    return 4;
  case FolderRole::Junk:
    return 5;
  case FolderRole::Trash:
    return 6;
  case FolderRole::Custom:
    return 7;
  }
  return 7;
}
...
QString FolderModel::iconNameFromRole(FolderRole role) {
  ...
  case FolderRole::Starred:
    return QStringLiteral("folder-important");
  case FolderRole::Sent:
    return QStringLiteral("mail-folder-sent");
  case FolderRole::Drafts:
...
void FolderModel::refresh() {
  ...
  {
    QSqlDatabase db =
        QSqlDatabase::addDatabase(QStringLiteral("QSQLITE"), DB_CONN);
    db.setDatabaseName(dbPath);
    db.setConnectOptions(QStringLiteral("QSQLITE_OPEN_READONLY"));

    if (!db.open()) {
      qWarning() << "FolderModel: failed to open cache.db:"
                 << db.lastError().text();
      rebuildTree();
      return;
    }
    ...
    QSqlQuery query(db);
    if (query.exec(
            QStringLiteral("SELECT account_id, path, unread_count, noselect, "
                           "kind FROM folders ORDER BY path"))) {
      while (query.next()) {
        RawFolder rf;
    ...
    if (acctQuery.exec(QStringLiteral("SELECT id, email FROM accounts"))) {
      while (acctQuery.next()) {
        newAccountEmails.insert(acctQuery.value(0).toInt(),
                                acctQuery.value(1).toString());
      }
    }
  ...
  for (const auto &rf : std::as_const(m_prevRawFolders)) {
    if (!rf.noselect)
      oldKeys.insert(QString::number(rf.accountId) + QLatin1Char(':') +
                     rf.path);
  }
  for (const auto &rf : std::as_const(newRawFolders)) {
    if (!rf.noselect)
      newKeys.insert(QString::number(rf.accountId) + QLatin1Char(':') +
                     rf.path);
  }
  if (oldKeys != newKeys)
...
void FolderModel::rebuildTree() {
  ...
  for (int acid : accountIds) {
    ...
    for (const auto &e : std::as_const(essentials))
      if (e.path.startsWith(QStringLiteral("[Gmail]/"))) {
        hasGmailNs = true;
        break;
      }
    if (hasGmailNs) {
      ...
      while (it != essentials.end()) {
        if (!it->path.startsWith(QStringLiteral("[Gmail]/"))) {
          customs.append(
              {it->accountId, it->path, it->unreadCount, false, it->kind});
  ...
  for (const auto &rf : std::as_const(m_rawFolders)) {
    ...
    if (!rf.path.startsWith(QStringLiteral("[Gmail]/"))) {
      ...
      for (const auto &other : std::as_const(m_rawFolders)) {
        if (other.accountId == rf.accountId &&
            other.path.startsWith(QStringLiteral("[Gmail]/"))) {
...
QModelIndex FolderModel::indexForPath(int accountId,
...
QModelIndex FolderModel::index(int row, int column,
...
QVariant FolderModel::data(const QModelIndex &index, int role) const {
  ...
  case IsAccountHeaderRole:
    return e.isAccountHeader;
  case EmailRole:
    return e.email;
  ...
  case IsEssentialRole:
    return e.isEssential;
  case FolderRoleRole:
    return static_cast<int>(e.role);
  }
  return {};
}
...
int FolderModel::count() const { return m_totalCount; }
